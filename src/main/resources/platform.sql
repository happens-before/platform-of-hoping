...
`status` tinyint(4) unsigned not null default '00' comment '00 全部 10 未发布 20 暂停 30已发布',
...
`valid` tinyint(1) not null default '0' comment '是否是员工',
`create_time` datetime not null default current_timestamp comment '创建时间',
`update_time` datetime not null default current_timestamp on update current_timestamp comment '更新时间',
primary key (`id`),
...
`valid` tinyint(1) not null default '0' comment '是否是组织者',
`password` varchar(20) not null default '123456' comment '密码',
`create_time` datetime not null default current_timestamp comment '创建时间',
`update_time` datetime not null default current_timestamp on update current_timestamp comment '更新时间',
primary key (`id`),
...
`valid` tinyint(1) UNSIGNED NOT NULL DEFAULT '0' COMMENT ' 0 1 ',
`activity_status` tinyint(4) unsigned not null default '00' comment '00 全部 10已结束的活动  20未发布  30进行中',
